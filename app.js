...
const SESSION_CACHE_BUSTER = "1.0";
...
function getTeamGeoHtml(teamName, isMen = true, showDist = true) {
    ...
    if (db.geo[key]) {
        ...
        let distanceHtml = '';
        if (showDist && dist && parseInt(dist) > 0 && !key.includes('КУТУЕВО')) {
...
function isGroupMatch(m) {
    ...
    return stage.includes('групп') || group !== '';
}
...
function mapServerData(data) {
    ...
    try {
        ...
        console.log("Ошибка маппинга данных:", e);
    }
}
...
function getGitHubLogoUrl(teamName) {
    ...
    if (!clean || clean === '' || clean === '---' || 
        ...
        clean.toUpperCase().includes('КОМАНДА №')) {
        return `https://cdn.jsdelivr.net/gh/ilnursultan/team-logos@main/logos/logo1.png?v=${SESSION_CACHE_BUSTER}`;
    }
    return `https://cdn.jsdelivr.net/gh/ilnursultan/team-logos@main/logos/${clean}.png?v=${SESSION_CACHE_BUSTER}`;
}

function getCombinedHistory() {
    let combined = [...db.archive];
    db.matches2026.filter(m => m.status === 'past').forEach(m => {
        let idInt = parseInt(m.id);
        let tournament = 'Мужчины';
        if (idInt >= 10000) tournament = 'Суперкубок';
        else if (idInt >= 1000) tournament = 'Женщины';
...
async function init() {
    ...
    try {
        ...
        if (!res || !res.matches2026) {
            throw new Error("Файл data.json пуст или поврежден.");
        }
        ...
        let initialMode = '2026';
        try {
            const hash = window.location.hash;
            if (hash === '#stats') { initialMode = 'stats'; }
            else if (hash === '#archive') { initialMode = 'archive'; }
            else if (hash === '#history') { initialMode = 'history'; }
        } catch(e) {} 
        ...
        if (animId) clearInterval(animId); 
        document.getElementById('loader').innerHTML = `<div class="text-red-500 font-bold text-sm">Ошибка загрузки: ${err.message}</div>`;
    }
}
...
function setupFiltersAndSelectors() {
    ...
    ['filter-teams', 'filter-stages', 'filter-status'].forEach(id => {
        ...
        if (el) { el.removeEventListener('change', render2026Schedule); el.addEventListener('change', render2026Schedule); }
    });
    updateArchiveYears();
}
...
function renderTeamStatistics() {
    ...
    function processMatch(m, yearStr) {
        ...
        if (isGroupMatch(m)) currentStage = 'Групповой этап';
...
function render2026Schedule() {
    ...
    let filtered = db.matches2026.filter(m => {
        ...
        if(sFilter === 'Групповой этап' && !isGroupMatch(m)) return false;
        if(sFilter === 'Плей-офф' && isGroupMatch(m)) return false;
    ...
    container.innerHTML = filtered.map((m) => {
        ...
        if (!isGroupMatch(m) && (t1Name === 'КОМАНДА' || t1Name === '---' || !t1Name)) { t1Name = getPlayoffFallbackLabel(m.id, 't1'); t2Name = getPlayoffFallbackLabel(m.id, 't2'); }
...
function render2026Playoffs() {
    ...
    const pMatches = db.matches2026.filter(m => {
        ...
        return idInt < 1000 && !isGroupMatch(m);
    });
    ...
    if (isSupercup) {
        ...
        pairsConfigWomen.forEach(cfg => {
            ...
            let cleanGroupName = (gValue) => (!gValue ? '' : gValue.toString().toUpperCase().replace('ГРУППА', '').trim());
        ...
        if (realFinal) matchesHtml += renderPlayoffCardLive(realFinal, 'ФИНАЛИСТ #1', 'ФИНАЛИСТ #2', '-', '-', 'Финал');
        
        const real3rd = pMatches.find(m => m.id === thirdPlaceId); 
        if (real3rd) matchesHtml += renderPlayoffCardLive(real3rd, isWomen ? 'ПРОИГРАВШАЯ 1/2 #1' : 'ПРОИГРАВШАЯ 1/2 #2', isWomen ? 'ПРОИГРАВШАЯ 1/2 #2' : 'ПРОИГРАВШИЙ 1/2 #2', '-', '-', 'Матч за 3 место');
    } else {
        let stageMatches = pMatches.filter(m => m.stage.includes(currentPlayoffStage2026));
...
function renderArchiveCore() {
    ...
    const gContainer = document.getElementById('archive-groups'); const pContainer = document.getElementById('archive-playoffs'); const rContainer = document.getElementById('archive-results'); const rInfo = document.getElementById('archive-rules-info');
    [gContainer, pContainer, rContainer, rInfo].forEach(c => c?.classList.add('hidden'));
    ...
    if(activeArchiveTab === 'groups') {
        ...
        const groupM = mArch.filter(isGroupMatch);
        ...
        const playoffTeams = new Set(mArch.filter(m => !isGroupMatch(m)).flatMap(m => [m.t1, m.t2]).filter(Boolean));
        groups.forEach(g => {
    ...
    if(activeArchiveTab === 'playoffs') {
        pContainer.classList.remove('hidden'); const playM = mArch.filter(m => !isGroupMatch(m));
...
// === РЕНДЕРИНГ ЗАЛА СЛАВЫ ===
function renderHistory() {
    const container = document.getElementById('history-content');
    if (!container) return;

    let combinedHistory = getCombinedHistory();
    let historyGroups = [];
    
    combinedHistory.forEach(m => {
        if (!historyGroups.find(g => g.year === m.year && g.tournament === m.tournament)) {
            historyGroups.push({ year: m.year, tournament: m.tournament });
        }
    });

    historyGroups.sort((a, b) => {
        if (b.year !== a.year) return parseInt(b.year) - parseInt(a.year);
        let weightA = a.tournament.toLowerCase().includes('мужчины') ? 1 : (a.tournament.toLowerCase().includes('женщины') ? 2 : 3);
        let weightB = b.tournament.toLowerCase().includes('мужчины') ? 1 : (b.tournament.toLowerCase().includes('женщины') ? 2 : 3);
        return weightA - weightB;
    });

    if (historyGroups.length === 0) {
        container.innerHTML = `<div class="text-zinc-600 text-xs text-center py-10 italic">Зал славы пуст</div>`;
        return;
    }
    ...
    historyGroups.forEach(group => {
        let year = group.year;
        let tournament = group.tournament;
        let isMen = tournament.toLowerCase().includes('мужчины');

        let marginClass = 'mb-3';
        if (previousYear !== null && previousYear !== year) {
            marginClass = 'mt-16 mb-3';
        }
        previousYear = year;

        let yMatches = combinedHistory.filter(m => m.year === year && m.tournament === tournament);
        
        let teamsSet = new Set();
        yMatches.forEach(m => {
            if (m.t1 && m.t1 !== '-' && m.t1 !== '') teamsSet.add(normalizeTeamName(m.t1));
            if (m.t2 && m.t2 !== '-' && m.t2 !== '') teamsSet.add(normalizeTeamName(m.t2));
        });
        let teamsCount = teamsSet.size;

        let finalMatch = yMatches.find(m => m.stage.toLowerCase().includes('финал') && !m.stage.toLowerCase().includes('3'));
        let thirdMatch = yMatches.find(m => m.stage.toLowerCase().includes('3-е место') || m.stage.toLowerCase().includes('3 место'));

        let champ = null, second = null, third = null;

        if (finalMatch) {
            if (finalMatch.s1 > finalMatch.s2 || (finalMatch.s1 === finalMatch.s2 && finalMatch.p1 > finalMatch.p2)) {
                champ = finalMatch.t1; second = finalMatch.t2;
            } else {
                champ = finalMatch.t2; second = finalMatch.t1;
            }
        }

        if (thirdMatch) {
            if (thirdMatch.s1 > thirdMatch.s2 || (thirdMatch.s1 === thirdMatch.s2 && thirdMatch.p1 > thirdMatch.p2)) {
                third = thirdMatch.t1;
            } else {
                third = thirdMatch.t2;
            }
        ...
        const renderTeamBadge = (tName, placeColor, icon, textColor) => {
            if (!tName) return '';
            let geoStr = '';
            const key = normalizeTeamName(tName).toUpperCase();
            if (db.geo[key]) {
                let item = db.geo[key];
                let dText = item.district || '';
                if (isMen && dText && !dText.toLowerCase().includes('район') && !dText.toLowerCase().includes('р-н')) { dText += ' район'; }
                geoStr = `${dText}${dText && item.subject ? ', ' : ''}${item.subject}`.toUpperCase();
            }

            return `
                <div class="flex items-center gap-3 bg-zinc-900/30 border border-zinc-800/50 p-3 rounded-2xl mb-2 relative overflow-hidden transition hover:bg-zinc-900/50">
                    <div class="absolute left-0 top-0 bottom-0 w-1 ${placeColor}"></div>
                    <div class="text-xl ml-2 w-6 text-center shrink-0 drop-shadow-md select-none">${icon}</div>
                    <div class="flex-1 min-w-0 flex items-center gap-3 pl-1">
                        <img src="${getGitHubLogoUrl(tName)}" class="w-10 h-10 object-contain drop-shadow-lg shrink-0" onerror="this.src='https://raw.githubusercontent.com/ilnursultan/team-logos/main/logos/standart.png'">
                        <div class="flex flex-col gap-0.5 min-w-0">
                            <span class="text-white font-black uppercase tracking-tight text-xs sm:text-sm truncate ${textColor}">${smartTeamName(tName)}</span>
                            <span class="text-zinc-500 text-[9px] font-bold uppercase tracking-wider truncate">${geoStr}</span>
                        </div>
                    </div>
                </div>
            `;
        ...
        html += `
            <div class="bg-zinc-card border border-zinc-800/80 rounded-[32px] p-5 shadow-2xl relative overflow-hidden ${marginClass}">
                <div class="absolute right-[-10px] top-[-10px] text-[120px] font-black text-white/[0.03] leading-none select-none pointer-events-none z-0">${year}</div>
                
                <div class="flex justify-between items-start mb-5 relative z-10">
                    <div class="pr-2">
                        <h3 class="text-3xl font-black text-white tracking-tight leading-none">${year}</h3>
                        <span class="text-[9px] text-neon font-bold uppercase tracking-widest mt-1.5 block">${tournament}</span>
                    </div>
                    <div onclick="openHistoryTeamsModal('${year}', '${tournament}')" class="bg-zinc-900/60 backdrop-blur border border-zinc-800 px-3 py-2 rounded-2xl text-center shadow-md cursor-pointer hover:bg-zinc-800/60 active:scale-95 transition-all shrink-0">
                        <span class="text-xl font-black text-white leading-none block">${teamsCount}</span>
                        <span class="text-[8px] text-zinc-500 uppercase font-black tracking-widest leading-none block mt-1">Команд</span>
                    </div>
                </div>
                
                <div class="flex flex-col relative z-10">
                    ${renderTeamBadge(champ, 'bg-[#FFD700]', '🥇', 'text-[#FFD700]')}
                    ${renderTeamBadge(second, 'bg-[#C0C0C0]', '🥈', 'text-white')}
                    ${renderTeamBadge(third, 'bg-[#CD7F32]', '🥉', 'text-white')}
                </div>
            </div>
        `;
    });
...
function openHistoryTeamsModal(year, tournament) {
    let combinedHistory = getCombinedHistory();
    let mArch = combinedHistory.filter(m => m.year === year && m.tournament === tournament);
    let teamsSet = new Set();
    ...
    let isMen = tournament.toLowerCase().includes('мужчины');
    let teamsArr = Array.from(teamsSet);
    
    let grouped = {};
    teamsArr.forEach(tName => {
        let key = tName.toUpperCase();
        let geoInfo = db.geo[key] || { subject: 'НЕИЗВЕСТНЫЙ РЕГИОН', district: 'БЕЗ РАЙОНА' };
        let subject = (geoInfo.subject || 'НЕИЗВЕСТНЫЙ РЕГИОН').toUpperCase();
        
        // Если женщины — районы вообще игнорируем, пишем в одну кучу
        let district = isMen ? (geoInfo.district || 'БЕЗ РАЙОНА') : 'ОБЩИЙ СПИСОК';
        
        if (isMen && district && !district.toLowerCase().includes('район') && !district.toLowerCase().includes('р-н') && district !== 'БЕЗ РАЙОНА') { 
            district += ' РАЙОН'; 
        }
        district = district.toUpperCase();
        
        if (!grouped[subject]) grouped[subject] = {};
        if (!grouped[subject][district]) grouped[subject][district] = [];
        grouped[subject][district].push(tName);
    });

    let html = '';
    
    // Сортировка регионов (Башкортостан всегда первый)
    Object.keys(grouped).sort((a, b) => {
        let isBashA = a.includes('БАШКОРТОСТАН');
        let isBashB = b.includes('БАШКОРТОСТАН');
        if (isBashA && !isBashB) return -1;
        if (!isBashA && isBashB) return 1;
        return a.localeCompare(b);
    }).forEach((subj) => {
        
        // Плотный, красивый заголовок РЕГИОНА (больше ничего не накладывается)
        html += `
        <div class="mt-6 mb-3 flex justify-start">
            <span class="bg-zinc-900 border border-zinc-800 text-neon px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-md">
                ${subj}
            </span>
        </div>`;
        
        // Сортировка районов / списков
        Object.keys(grouped[subj]).sort((a, b) => {
            let isUchA = a.includes('УЧАЛИНСКИЙ');
            let isUchB = b.includes('УЧАЛИНСКИЙ');
            if (isUchA && !isUchB) return -1;
            if (!isUchA && isUchB) return 1;
            return a.localeCompare(b);
        }).forEach(dist => {
            
            // Показываем подзаголовок района ТОЛЬКО у мужчин и если это не "Без района"
            if (isMen && dist !== 'БЕЗ РАЙОНА') {
                html += `<div class="text-[10px] text-zinc-400 font-black uppercase tracking-wider mt-3 mb-1.5 pl-2">${dist}</div>`;
            }
            
            html += `<div class="bg-zinc-900/40 border border-zinc-800/60 rounded-2xl flex flex-col shadow-inner divide-y divide-zinc-800/40 mb-3">`;
            
            // Сортировка команд внутри (Кутуево вверху)
            let sortedTeams = grouped[subj][dist].sort((a, b) => {
                let isKutA = a.toUpperCase().includes('КУТУЕВО');
                let isKutB = b.toUpperCase().includes('КУТУЕВО');
                if (isKutA && !isKutB) return -1;
                if (!isKutA && isKutB) return 1;
                return a.localeCompare(b);
            });
            ...
            sortedTeams.forEach(t => {
                html += `
                <div class="flex items-center gap-3 p-3 hover:bg-zinc-800/50 transition-colors">
                    <img src="${getGitHubLogoUrl(t)}" class="w-6 h-6 object-contain drop-shadow-md" onerror="this.src='https://raw.githubusercontent.com/ilnursultan/team-logos/main/logos/standart.png'">
                    <span class="text-xs sm:text-sm font-bold text-white uppercase tracking-tight">${smartTeamName(t)}</span>
                </div>`;
            });
            html += `</div>`;
        });
    ...
    document.getElementById('history-teams-modal-title').innerText = `Команды: ${year}`;
    document.getElementById('history-teams-modal-list').innerHTML = html;
    document.getElementById('history-teams-modal').classList.remove('hidden');
}
function closeHistoryTeamsModal() {
    document.getElementById('history-teams-modal').classList.add('hidden');
}

function setGlobalMode(mode) {
    currentGlobalMode = mode;
    const view2026 = document.getElementById('view-2026'); 
    const viewArch = document.getElementById('view-archive'); 
    const viewStats = document.getElementById('view-stats');
    const viewHist = document.getElementById('view-history');

    const modeContainer = document.getElementById('mode-container');

    [view2026, viewArch, viewStats, viewHist].forEach(v => { if (v) v.classList.add('hidden'); });
    ['mode-btn-2026', 'mode-btn-stats', 'mode-btn-archive', 'mode-btn-history'].forEach(id => { 
        let el = document.getElementById(id);
        if (el) el.className = "flex flex-col items-center justify-center w-1/4 py-2 text-gray-500 hover:text-white transition-colors outline-none cursor-pointer"; 
    });
    ...
    const tabs2026 = document.getElementById('nav-2026-tabs')?.parentElement; 
    const ctrlsArch = document.getElementById('archive-controls');
    ...
    if(mode === '2026') {
        ...
        document.getElementById('mode-btn-2026').className = "flex flex-col items-center justify-center w-1/4 py-2 text-neon transition-colors outline-none cursor-pointer";
        render2026Core();
        ...
        document.getElementById('mode-btn-archive').className = "flex flex-col items-center justify-center w-1/4 py-2 text-neon transition-colors outline-none cursor-pointer";
        renderArchiveCore();
    } else if (mode === 'stats') {
        if (viewStats) viewStats.classList.remove('hidden'); 
        document.getElementById('mode-btn-stats').className = "flex flex-col items-center justify-center w-1/4 py-2 text-neon transition-colors outline-none cursor-pointer";
        renderTeamStatistics();
    } else if (mode === 'history') {
        if (viewHist) viewHist.classList.remove('hidden'); 
        document.getElementById('mode-btn-history').className = "flex flex-col items-center justify-center w-1/4 py-2 text-neon transition-colors outline-none cursor-pointer";
        renderHistory();
    }
    ...
    if (mode === 'stats' || mode === 'history') {
        ...
        if (archiveIndicator) archiveIndicator.style.setProperty('display', 'none', 'important');
        if (scrollIndicator) scrollIndicator.style.setProperty('display', 'flex', 'important');
    }
}
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    setTimeout(() => {
        if(db.matches2026.length > 0 || db.archive.length > 0) setGlobalMode(initialMode);
    }, 100);
});
...
setGlobalMode = function(mode) {
    try {
        if (mode === 'stats') { window.location.hash = 'stats'; } 
        else if (mode === 'archive') { window.location.hash = 'archive'; } 
        else if (mode === 'history') { window.location.hash = 'history'; } 
        else if (mode === '2026') { history.replaceState(null, null, window.location.pathname); }
    } catch(e) {}
