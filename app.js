...
function openHistoryTeamsModal(year, tournament) {
    ...
    teamsArr.forEach(tName => {
        ...
        let district = geoInfo.district || 'Без района';
    ...
    Object.keys(grouped).sort((a, b) => {
    ...
    }).forEach((subj, subjIdx) => {
        let marginTop = subjIdx === 0 ? '' : 'mt-10'; 
        
        html += `<div class="${marginTop} relative">`;
        html += `
        <div class="sticky top-0 z-20 pt-2 pb-1 -mx-2 px-2 flex justify-start pointer-events-none">
            <span class="bg-[#111114] border border-zinc-700/80 text-neon px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-xl pointer-events-auto">
                ${subj}
            </span>
        </div>`;
        ...
        Object.keys(grouped[subj]).sort((a, b) => {
            ...
            let dName = dist !== 'БЕЗ РАЙОНА' ? dist : '';
            if (dName) {
                html += `<div class="text-[10px] text-white font-black uppercase tracking-wider mt-4 mb-2 pl-2">${dName}</div>`;
            }
            
            html += `<div class="bg-zinc-900/40 border border-zinc-800/60 rounded-2xl flex flex-col shadow-inner divide-y divide-zinc-800/40">`;
