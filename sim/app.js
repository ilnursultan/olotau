...
function openPlayoffModal(matchId, t1Fallback, t2Fallback) {
    ...
    document.getElementById('playoff-modal-penalty').classList.add('hidden');
        ...
        if (mGoals.length > 0) {
            ...
            mGoals.forEach(g => {
                ...
                if (isT1) {
                    ...
                    let reverseAssistant = g.assistant ? `<span class="text-zinc-500 text-[9px] mr-1">${shortenPlayerName(g.assistant)} → </span>` : '';
                    html += `
                    <div class="flex items-center py-1 border-b border-zinc-900/20 last:border-0 justify-end w-full text-right">
                        ${reverseAssistant}<span class="text-white font-bold text-[10px] truncate mr-2">${pName}</span>
                        <span class="text-neon font-black text-[10px] bg-zinc-900/80 px-1.5 rounded mr-2 shrink-0">${c1}:${c2}</span>
...
function renderPlayoffCardLive(m, t1Fallback, t2Fallback, gridPos1, gridPos2) {
    ...
    return `
    <div ${inlineStyle} class="match-row clickable-match bg-zinc-card border border-zinc-900 rounded-2xl p-3 flex justify-between items-center w-full relative overflow-hidden transition-all hover:border-zinc-800/80 cursor-pointer" onclick="openPlayoffModal('${matchId}', '${t1Fallback.replace(/'/g, "\\'")}', '${t2Fallback.replace(/'/g, "\\'")}')">
