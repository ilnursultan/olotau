...
async function initAdmin() {
    try {
        const [rM, rG, rP, rL] = await Promise.all([
            fetch(ADMIN_URLS.matches + '&cb=' + Date.now()).then(r => r.text()),
            fetch(ADMIN_URLS.goals + '&cb=' + Date.now()).then(r => r.text()),
            fetch(ADMIN_URLS.players + '&cb=' + Date.now()).then(r => r.text()),
            fetch(ADMIN_URLS.loats + '&cb=' + Date.now()).then(r => r.text())
        ]);
        db.matches2026 = parseCoreCSV(rM, 'm'); db.goals2026 = parseCoreCSV(rG, 'g'); db.players2026 = parseCoreCSV(rP, 'p'); parseLoatsCSV(rL);
