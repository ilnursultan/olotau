...
const APPS_SCRIPT_WEB_APP_URL = "https://script.google.com/macros/s/AKfycbzOvbAp3g-vYP1xz1qMJ3UbX1wGUhQ6VMj3AKjR7_BgZ_rLAFV2J2pWPLS1-yCSeht9/exec";
...
function render2026Playoffs() {
    ...
    if (currentPlayoffStage2026 === '1/16') {
        ...
        const realFinal = stageMatches.find(m => m.id === '601'); matchesHtml += renderPlayoffCardLive(realFinal, 'ФИНАЛИСТ #1', 'ФИНАЛИСТ #2', '-', '-');
        const real3rd = pMatches.find(m => m.id === '501'); matchesHtml += `<div class="mt-1.5 pt-2"><div class="text-[9px] font-black text-zinc-500 uppercase tracking-widest mb-1 pl-1">Матч за 3-е место</div></div>` + renderPlayoffCardLive(real3rd, 'ПРОИГРАВШИЙ 1/2 #1', 'ПРОИГРАВШИЙ 1/2 #2', '-', '-');
    } else {
...
function renderPlayoffCardLive(m, t1Fallback, t2Fallback, gridPos1, gridPos2) {
    ...
    let p1 = (isPast && m.p1 !== null) ? `<span class="text-white text-[9px] ml-1">${m.p1}</span>` : '';
    let p2 = (isPast && m.p2 !== null) ? `<span class="text-white text-[9px] ml-1">${m.p2}</span>` : '';
    ...
    return `<div class="bg-zinc-card border border-zinc-900 rounded-2xl p-3 flex justify-between items-center w-full relative overflow-hidden transition-all hover:border-zinc-800/80">
        <div class="flex-1 space-y-2 min-w-0 pr-2 z-10">
            <div class="flex items-center truncate text-[11px] sm:text-xs font-bold text-white uppercase">${gridBadge1}<img src="${getGitHubLogoUrl(t1Name)}" class="team-logo mr-2" onerror="this.src='https://raw.githubusercontent.com/ilnursultan/team-logos/main/logos/standart.png'"><span class="truncate ${isMock1 ? 'text-zinc-600 font-medium' : ''}">${smartTeamName(t1Name)}</span></div>
            <div class="flex items-center truncate text-[11px] sm:text-xs font-bold text-white uppercase">${gridBadge2}<img src="${getGitHubLogoUrl(t2Name)}" class="team-logo mr-2" onerror="this.src='https://raw.githubusercontent.com/ilnursultan/team-logos/main/logos/standart.png'"><span class="truncate ${isMock2 ? 'text-zinc-600 font-medium' : ''}">${smartTeamName(t2Name)}</span></div>
        </div>
...
function renderPlayoffCard(m, t1, t2) {
    ...
    let p1 = m.p1 !== null ? `<span class="text-white text-[9px] ml-1">${m.p1}</span>` : '';
    let p2 = m.p2 !== null ? `<span class="text-white text-[9px] ml-1">${m.p2}</span>` : '';
    ...
    return `<div class="bg-zinc-card border border-zinc-900 rounded-2xl p-3 flex justify-between items-center w-full relative overflow-hidden">
        ...
        <div class="flex flex-col gap-1.5 items-end justify-center shrink-0 z-10 font-black text-xs sm:text-sm">
            <div class="${m.s1 > m.s2 ? 'text-neon' : 'text-zinc-300'} flex items-center">${s1}${p1}</div>
            <div class="${m.s2 > m.s1 ? 'text-neon' : 'text-zinc-300'} flex items-center">${s2}${p2}</div>
        </div>
    </div>`;
}
        ...
        <div class="bg-zinc-900/30 border border-zinc-800/60 rounded-xl p-3 flex items-center justify-between mb-2 hover:bg-zinc-900/60 transition-colors">
            <div class="flex items-center gap-3">
                ...
                <div class="flex flex-col gap-0.5">
                    <span class="text-white font-black text-xs uppercase">${b.name}</span>
