...
if (bestStagesMap['ЧЕМПИОН 🏆']) { badgesHtml += `<div class="flex items-center gap-1 bg-amber-500/10 border border-amber-500/30 px-2.5 py-1 rounded-full text-[#FFD700] text-[10px] font-black shadow-[0_0_10px_rgba(255,215,0,0.2)]">🥇 ЧЕМПИОН <span class="opacity-70 font-bold ml-0.5">x</span>${bestStagesMap['ЧЕМПИОН 🏆'].length}</div>`; }
...
function renderPlayoffCard(m, t1, t2) {
    ...
    return `<div class="bg-zinc-card border border-zinc-900 rounded-2xl p-3 flex justify-between items-center w-full relative overflow-hidden">
        ...
        <div class="flex flex-col gap-1.5 items-end justify-center shrink-0 z-10 font-black text-[11px] sm:text-xs">
            ...
            <div class="text-neon flex items-center">${s2}${p2}</div>
        </div>
    </div>`;
}
